...
## $(date +%Y-%m-%d) - Eliminate LINQ allocations in MixedRouting.cs hot paths
**Learning:** Found that `MixedRouting.cs` heavily used LINQ extensions (`.ToDictionary`, `.Select`, `.Where`, `.Sum`, etc.) during context building and route capacity evaluations (`Allocate`, `RankStochastic`, `BuildStaticContext`, etc.). These repeated hidden enumerators and closure delegates cause massive CPU and Garbage Collection bottlenecks inside the engine's nested simulation loops, significantly increasing runtimes.
**Action:** Unrolled multiple LINQ method chains into explicit standard `foreach` and `for` loops across the `MixedRoutingAllocator`. Manually pre-allocated `Dictionary` sizes using collection `.Count` to reduce overhead and swapped chained aggregations (`.Sum()`, `.Min()`) for fast linear accumulator patterns.
