...
public sealed class NetworkSimulationEngine
{
    ...
    private static void AnalyzeContextResults(NetworkModel network, IReadOnlyList<RoutingTrafficContext> contexts, bool hasFiniteCapacities)
    {
        foreach (var context in contexts)
        {
            ...
            // Bolt: Replaced LINQ .Sum() with manual foreach loops to eliminate delegate and enumerator allocations on a hot path.
            ...
            // Bolt: Replaced LINQ .Sum() with a manual foreach loop to avoid delegate allocations.
