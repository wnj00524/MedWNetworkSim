...
public sealed class WorkspaceViewModel : ObservableObject, IUiExceptionSink, ICanvasSelectionService, IElementEditorCoordinator
{
    ...
    private SimulationActorMetrics CreateEconomicMetrics(TrafficEconomicSettlementResult settlement)
    {
        ...
        var actorCount = SimulationActors.Count;
        var actorCashById = new Dictionary<string, double>(actorCount, Comparer);
        var actorSalesRevenueById = new Dictionary<string, double>(actorCount, Comparer);
        var actorPurchaseCostById = new Dictionary<string, double>(actorCount, Comparer);
        var actorProductionCostById = new Dictionary<string, double>(actorCount, Comparer);
        var actorTransportCostById = new Dictionary<string, double>(actorCount, Comparer);
        var actorTaxesPaidById = new Dictionary<string, double>(actorCount, Comparer);
        var actorTaxesReceivedById = new Dictionary<string, double>(actorCount, Comparer);
        var actorCashDeltaById = new Dictionary<string, double>(actorCount, Comparer);
        var actorProfitById = new Dictionary<string, double>(actorCount, Comparer);

        double totalCooperationWeight = 0d;
        ...
        foreach (var actor in SimulationActors)
        {
            ...
            totalCooperationWeight += Math.Clamp(actor.CooperationWeight, 0d, 1d);
        }
        ...
        return new SimulationActorMetrics
        {
            ...
            CooperationIndex = actorCount == 0 ? 0d : totalCooperationWeight / actorCount
        };
    }
